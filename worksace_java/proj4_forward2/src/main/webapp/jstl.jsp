...
기본적으로 el은 page, request, session, application 순으로 찾음 없으면 null<br>
\${game } : ${game }<br>
...
<c:choose>
	<c:when test="${game eq 'lol' }">롤 유저</c:when>
	<c:when test="${game eq 'overwatch' }">옵치 유저</c:when>
...
end가 items 개수보다 커도 items 만큼 돌고 오류 없음<br>
<c:forEach var="map" items="${list2}" begin="0" end="10" step="3">
	${map}<br>
</c:forEach>
<hr>
<a href="/jstl.jsp?name=한글">jstl.jsp?name=한글<br></a>
...
<br>
c:url을 사용하는 이유<br>
1. 영어나 숫자 외의 한글이나 특수문자를 encode 해줌
2. context path 자동 추가(/_proj4_froward)<br>
